from GHZ_pauli_wire_cut import GHZPauliWireCut
from quantum_wire_cutting import QuantumWireCutUtils as qu

# Test the performance of the Pauli-based approach with parameters
# qubits = 5; cuts = 1;

ERROR_THRESHOLD = 0.1

def test_performance_pauli_based_5_1_1000():

    passedCount = 0
    failedCount = 0
    for i in range(100):
        exp = GHZPauliWireCut(5, 1, 1000)
        orgProbs = exp.RunOriginialCircuit()
        shots = exp.DoSubCircuitMeasurements()
        cutProbs = exp.ConstructFullProbabilites()
        if qu.EvaluateProbs(orgProbs, cutProbs, ERROR_THRESHOLD):
            passedCount += 1
        else:
            failedCount += 1

    assert passedCount > failedCount


def test_performance_pauli_based_5_1_500():
    passedCount = 0
    failedCount = 0
    for i in range(100):
        exp = GHZPauliWireCut(5, 1, 500)
        orgProbs = exp.RunOriginialCircuit()
        shots = exp.DoSubCircuitMeasurements()
        cutProbs = exp.ConstructFullProbabilites()
        if qu.EvaluateProbs(orgProbs, cutProbs, ERROR_THRESHOLD):
            passedCount += 1
        else:
            failedCount += 1

    assert passedCount > failedCount


def test_performance_pauli_based_5_1_250():
    passedCount = 0
    failedCount = 0
    for i in range(100):
        exp = GHZPauliWireCut(5, 1, 250)
        orgProbs = exp.RunOriginialCircuit()
        shots = exp.DoSubCircuitMeasurements()
        cutProbs = exp.ConstructFullProbabilites()
        if qu.EvaluateProbs(orgProbs, cutProbs, ERROR_THRESHOLD):
            passedCount += 1
        else:
            failedCount += 1

    assert passedCount > failedCount


def test_performance_pauli_based_5_1_100():
    passedCount = 0
    failedCount = 0
    for i in range(100):
        exp = GHZPauliWireCut(5, 1, 100)
        orgProbs = exp.RunOriginialCircuit()
        shots = exp.DoSubCircuitMeasurements()
        cutProbs = exp.ConstructFullProbabilites()
        if qu.EvaluateProbs(orgProbs, cutProbs, ERROR_THRESHOLD):
            passedCount += 1
        else:
            failedCount += 1

    assert not passedCount > failedCount
